...
alter table if exists "ConsentRecord" enable row level security;
alter table if exists "Swipe" enable row level security;
alter table if exists "RecommendationExposure" enable row level security;
alter table if exists "RoomShareLink" enable row level security;
alter table if exists "IntentOffline" enable row level security;
...
drop policy if exists "Read preferences (auth users)" on "Preference";
create policy "Read preferences (auth users)"
on "Preference"
for select
using (auth.uid() is not null);
...
-- Conversation participants (insert via server only).
drop policy if exists "Insert conversation participants" on "ConversationParticipant";

-- Matches (read only on client).
...
drop policy if exists "Insert own matches" on "Match";
drop policy if exists "Upsert own matches" on "Match";
...
-- Swipes (read only on client).
drop policy if exists "Read own swipes" on "Swipe";
create policy "Read own swipes"
on "Swipe"
for select
using (auth.uid()::text = "fromUserId" or auth.uid()::text = "toUserId");

drop policy if exists "Upsert own swipes" on "Swipe";
drop policy if exists "Update own swipes" on "Swipe";

-- Recommendation exposures.
drop policy if exists "Read own exposures" on "RecommendationExposure";
create policy "Read own exposures"
on "RecommendationExposure"
for select
using (auth.uid()::text = "viewerId");

drop policy if exists "Upsert own exposures" on "RecommendationExposure";
create policy "Upsert own exposures"
on "RecommendationExposure"
for insert
with check (auth.uid()::text = "viewerId");

drop policy if exists "Update own exposures" on "RecommendationExposure";
create policy "Update own exposures"
on "RecommendationExposure"
for update
using (auth.uid()::text = "viewerId")
with check (auth.uid()::text = "viewerId");

-- Consent records (read only on client).
drop policy if exists "Read match consent" on "ConsentRecord";
create policy "Read match consent"
on "ConsentRecord"
for select
using (
  exists (
    select 1
    from "Match" m
    where m.id = "ConsentRecord"."matchId"
      and (m."user1Id" = auth.uid()::text or m."user2Id" = auth.uid()::text)
  )
);

drop policy if exists "Insert match consent" on "ConsentRecord";
drop policy if exists "Update match consent" on "ConsentRecord";

-- Intent offline (read only on client).
drop policy if exists "Read intent offline" on "IntentOffline";
create policy "Read intent offline"
on "IntentOffline"
for select
using (
  auth.uid()::text = "requesterId" or auth.uid()::text = "responderId"
);

drop policy if exists "Insert intent offline" on "IntentOffline";
drop policy if exists "Update intent offline" on "IntentOffline";

-- Room share links.
drop policy if exists "Read room share links" on "RoomShareLink";
create policy "Read room share links"
on "RoomShareLink"
for select
using (auth.uid() is not null);

-- Room membership updates.
drop policy if exists "Update own membership" on "RoomMembership";
...
drop policy if exists "Insert own report" on "Report";
-- Report inserts via server only.
